...
#           GA方針
#   枠内の配置しうる座標からランダムに抽出し
#   その座標を各ピースの頂点0に割り当てそれをもとに描画し（表示はしない）
#   全座標の画素値にアクセスして色値が255以上（白のpixel×3）の数を数えることで評価する
#   その作業をピース個回繰り返し、評価の高かった頂点0の座標、色値の数、その時のピースの角度を保存する

#   それをもとにし、次からはピースを1つ動かして評価するを繰り返し、
#   動かすピースとその座標、角度を保存し、世代を進化させる。それがエリート世代となり
#   次世代を作成する。
#   この次世代生成をとりうる白の値の8割ぐらいとなるまで繰り返す
#
